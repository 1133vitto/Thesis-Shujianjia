"""
极简重构版：雷达-LiDAR 融合检测项目
核心优化：
1. 引入 einops 解决复杂的维度变换，代码更易读
2. 引入 segmentation_models_pytorch (smp) 替代手写 U-Net，大幅精简代码
3. 修复了原代码中背景噪声估计可能为负数的物理逻辑错误
4. 修复了 Dataset 中点云长度不一导致 DataLoader 崩溃的 Bug
"""
...
class RadarEncoder(nn.Module):
    """
    轻量级雷达编码器 (OOM 终结者版)
    核心逻辑：不拆分空间维度，而是直接把 Doppler 看作输入特征的 Channels。
    """
...
class FastFusionModel(nn.Module):
    ...
    """
    def __init__(self, angle_bins: int = 256, cnn_out_channels: int = 64):
        super().__init__()
